...
import "../styles/frequency-range.css";
...
class Scales extends Component {
  ...
  remove_event(e) {
    ...
    let newObj = this.state.evCache;
    let key = e.pointerId || e.identifier;
    delete newObj[key];
  ...
  onTouchStart(e) {
    ...
    for (let i = 0; i < e.changedTouches.length; i++) {
      let key = e.changedTouches[i].identifier;
      let newObj = this.state.evCache;
  ...
  onTouchMove(e) {
    console.log("MOVE")
      ...
      for (let i = 0; i < e.touches.length; i++) {
        let key = e.touches[i].identifier;
  ...
  onTouchEnd(e) {
    for(let i=0; i<e.changedTouches.length; i++){
      this.remove_event(e.changedTouches[i]);
    }
    let length = Object.keys(this.state.evCache).length;
    let {height, width} = this.props;
    this.ctx.clearRect(0, 0, width, height);
